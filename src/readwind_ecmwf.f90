...
!FDB variables
  integer :: validityTime, validityDate ! the values to fetch from fdb
  integer :: message_count !fdb count of messages returned 
  integer(c_long) :: total_size, read, marker, messageLength
...
!  readcloud=.false.
  ...
  message_count = 0
  call start_timer(timer_readwind_loop)
  ...
  call stop_timer(timer_readwind_loop)
  write(*,*) 'Read ', message_count, ' GRIB2 messages.'
...
subroutine expand_values_for_request(expanded_list,start,end)
  integer :: var
  integer :: start, end
  character(len=*) :: expanded_list(*)
  do var = start, end
    write(expanded_list(var-start+1), '(I6.1)') var
    expanded_list(var-start+1)=adjustl(trim(expanded_list(var-start+1)))
  end do
end subroutine expand_values_for_request
